...
@section('description', 'Create a new influencer collaboration')
@section('page-title', 'Create Collaboration')
...
<div class="fade-in">
    ...
    <div style="margin-bottom: 2rem;">
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                ...
                <p style="color: var(--text-secondary);">Add a new collaboration opportunity to showcase partnership possibilities and attract brands.</p>
            </div>
    ...
    <form method="POST" action="{{ route('influencer.collaborations.store') }}" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 2rem;">
        @csrf
            ...
            <div class="card-body">
                ...
                <div style="margin-top: 1.5rem;">
                    ...
                    <textarea id="brief" name="brief" rows="3"
                              style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.375rem; font-size: 0.875rem; transition: all 0.2s; resize: vertical;">{{ old('brief') }}</textarea>
            ...
            <div class="card-body">
                    ...
                    <div>
                        ...
                        <select id="collaboration_mode" name="collaboration_mode" required
                            ...
                            @foreach($modes as $key => $label)
                                <option value="{{ $key }}" {{ old('collaboration_mode') == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-top: 1.5rem;">
                    <div>
                        ...
                        <input type="number" id="budget_min" name="budget_min" step="0.01" min="0" value="{{ old('budget_min') }}"
                               style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.375rem; font-size: 0.875rem; transition: all 0.2s;">
                        @error('budget_min')
                            <p style="color: var(--danger-color); font-size: 0.75rem; margin-top: 0.25rem;">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="number" id="budget_max" name="budget_max" step="0.01" min="0" value="{{ old('budget_max') }}"
                               style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.375rem; font-size: 0.875rem; transition: all 0.2s;">
                        @error('budget_max')
                            <p style="color: var(--danger-color); font-size: 0.75rem; margin-top: 0.25rem;">{{ $message }}</p>
                        @enderror
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                    @foreach($platforms as $key => $label)
                        <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.375rem; transition: all 0.2s;">
                            <input type="checkbox" name="social_platforms[]" value="{{ $key }}" {{ in_array($key, old('social_platforms', [])) ? 'checked' : '' }}
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                    <div>
                        ...
                        <input type="file" id="thumbnail_image" name="thumbnail_image" accept="image/*"
                               style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.375rem; font-size: 0.875rem; transition: all 0.2s;">
                        <p style="color: var(--text-secondary); font-size: 0.75rem; margin-top: 0.25rem;">Main image for your collaboration (max 2MB)</p>
        ...
        <div style="display: flex; justify-content: flex-end; gap: 1rem;">
            ...
            <button type="submit" 
                ...
                <i class="fas fa-save" style="margin-right: 0.5rem;"></i>Create Collaboration
            </button>
        </div>
    </form>
</div>
