...
@section('description', 'Edit package details')
...
<div class="fade-in">
    ...
    <div style="margin-bottom: 2rem;">
        <div style="display: flex; align-items: center; justify-content: space-between;">
            <div>
                ...
                <p style="color: var(--text-secondary);">Update package details and information.</p>
            </div>
    ...
    <form method="POST" action="{{ route('influencer.packages.update', $package) }}" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 2rem;">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                    ...
                    <div>
                        ...
                        <select id="package_type" name="package_type" required
                            ...
                            @foreach($packageTypes as $key => $label)
                                <option value="{{ $key }}" {{ old('package_type', $package->package_type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('package_type')
                            <p style="color: var(--danger-color); font-size: 0.75rem; margin-top: 0.25rem;">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select id="category" name="category" required
                            ...
                            @foreach($categories as $key => $label)
                                <option value="{{ $key }}" {{ old('category', $package->category) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('category')
                            <p style="color: var(--danger-color); font-size: 0.75rem; margin-top: 0.25rem;">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select id="pricing_model" name="pricing_model" required
                            ...
                            @foreach($pricingModels as $key => $label)
                                <option value="{{ $key }}" {{ old('pricing_model', $package->pricing_model) == $key ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('pricing_model')
                            <p style="color: var(--danger-color); font-size: 0.75rem; margin-top: 0.25rem;">{{ $message }}</p>
                ...
                <div style="margin-top: 1.5rem;">
                    ...
                    <textarea id="description" name="description" rows="4" required
                              style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.375rem; font-size: 0.875rem; transition: all 0.2s; resize: vertical;">{{ old('description', $package->description) }}</textarea>
                    @error('description')
                        <p style="color: var(--danger-color); font-size: 0.75rem; margin-top: 0.25rem;">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="card-body">
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1.5rem;">
                    <div>
                        ...
                        <input type="number" id="post_count" name="post_count" min="0" value="{{ old('post_count', $package->post_count) }}"
                               style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.375rem; font-size: 0.875rem; transition: all 0.2s;">
                    ...
                    <div>
                        ...
                        <input type="number" id="story_count" name="story_count" min="0" value="{{ old('story_count', $package->story_count) }}"
                               style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.375rem; font-size: 0.875rem; transition: all 0.2s;">
                    ...
                    <div>
                        ...
                        <input type="number" id="reel_count" name="reel_count" min="0" value="{{ old('reel_count', $package->reel_count) }}"
                               style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 0.375rem; font-size: 0.875rem; transition: all 0.2s;">
            ...
            <div class="card-body">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.375rem; transition: all 0.2s;">
                        <input type="checkbox" name="social_platforms[]" value="instagram" {{ in_array('instagram', old('social_platforms', $package->social_platforms ?? [])) ? 'checked' : '' }}
                               style="width: 1rem; height: 1rem;">
                        <span style="font-weight: 500; color: var(--text-primary);">Instagram</span>
                    </label>
                    
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.375rem; transition: all 0.2s;">
                        <input type="checkbox" name="social_platforms[]" value="facebook" {{ in_array('facebook', old('social_platforms', $package->social_platforms ?? [])) ? 'checked' : '' }}
                               style="width: 1rem; height: 1rem;">
                        <span style="font-weight: 500; color: var(--text-primary);">Facebook</span>
                    </label>
                    
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.375rem; transition: all 0.2s;">
                        <input type="checkbox" name="social_platforms[]" value="twitter" {{ in_array('twitter', old('social_platforms', $package->social_platforms ?? [])) ? 'checked' : '' }}
                               style="width: 1rem; height: 1rem;">
                        <span style="font-weight: 500; color: var(--text-primary);">Twitter</span>
                    </label>
                    
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.375rem; transition: all 0.2s;">
                        <input type="checkbox" name="social_platforms[]" value="tiktok" {{ in_array('tiktok', old('social_platforms', $package->social_platforms ?? [])) ? 'checked' : '' }}
                               style="width: 1rem; height: 1rem;">
                        <span style="font-weight: 500; color: var(--text-primary);">TikTok</span>
                    </label>
                    
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.375rem; transition: all 0.2s;">
                        <input type="checkbox" name="social_platforms[]" value="youtube" {{ in_array('youtube', old('social_platforms', $package->social_platforms ?? [])) ? 'checked' : '' }}
                               style="width: 1rem; height: 1rem;">
                        <span style="font-weight: 500; color: var(--text-primary);">YouTube</span>
                    </label>
                    
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.375rem; transition: all 0.2s;">
                        <input type="checkbox" name="social_platforms[]" value="linkedin" {{ in_array('linkedin', old('social_platforms', $package->social_platforms ?? [])) ? 'checked' : '' }}
                               style="width: 1rem; height: 1rem;">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <!-- Current Images -->
                @if($package->thumbnail_image || ($package->gallery_images && count($package->gallery_images) > 0))
                    <div style="margin-bottom: 1.5rem;">
                        ...
                        @if($package->thumbnail_image)
                            <div style="margin-bottom: 1rem;">
                                ...
                                <img src="{{ $package->thumbnail_url }}" alt="Current thumbnail" 
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                    ...
                    <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 0.375rem; transition: all 0.2s;">
                        <input type="checkbox" name="is_custom" value="1" {{ old('is_custom', $package->is_custom) ? 'checked' : '' }}
                               style="width: 1rem; height: 1rem;">
        ...
        <div style="display: flex; justify-content: flex-end; gap: 1rem;">
            ...
            <button type="submit" 
                ...
                <i class="fas fa-save" style="margin-right: 0.5rem;"></i>Update Package
            </button>
        </div>
    </form>
</div>
